...
export const openExamPrepQuestions = [
  ...
  complexQ({
    ...
    correct: { en: "Groups bind sets of permissions and record domains (ir.access) to users", fr: "Les groupes lient des ensembles de permissions et de domaines d'enregistrement (ir.access) aux utilisateurs" },
    distractors: [
    ...
    explanation: { en: "res.groups is the security group model. From Odoo 19.4, ACLs and former record rules are unified in ir.access: each access line has a model, CRUD operations, an optional group, and a domain. The 'Own Documents Only' vs 'All Documents' pattern in Sales is an access domain that restricts a user to their own opportunities/quotations unless they are in the all-documents group.\n\nWhy not \"Groups are only labels\"? Groups are functional, not just labels.\n\nWhy not \"Groups cannot restrict records\"? Domains on access rights absolutely restrict records.\n\nWhy not \"Groups apply only to inventory\"? Groups apply to every model, not just inventory.", fr: "res.groups est le modèle de groupe de sécurité. Depuis Odoo 19.4, ACL et anciennes règles d'enregistrement sont unifiés dans ir.access : chaque ligne a un modèle, des opérations CRUD, un groupe optionnel et un domaine. Le modèle « Documents personnels uniquement » vs « Tous les documents » dans Ventes est un domaine d'accès." },
  }),
  complexQ({
    id: "oep-092",
    module: "studio",
  ...
  complexQ({
    ...
    correct: { en: "Access right (ir.access) with a domain on crm.lead scoped to the user group", fr: "Droit d'accès (ir.access) avec un domaine sur crm.lead rattaché au groupe d'utilisateurs" },
    distractors: [
    ...
    explanation: { en: "From Odoo 19.4, record-level filters live on ir.access (domain on the access right), not a separate ir.rule. Example: country_id matching the user's country for a sales group. Permissions and row filters are configured in one place.\n\nWhy not \"Email template QWeb placeholder replacing merge fields\"? Email templates format outbound messages; they do not enforce record-level security.\n\nWhy not \"Inventory AVCO average cost recalculation on receipts\"? AVCO valuation recalculates inventory cost, unrelated to CRM record access.\n\nWhy not \"Website snippet drag-and-drop block layout editor\"? Website snippets design pages; they do not filter CRM opportunity records.", fr: "Depuis Odoo 19.4, les filtres au niveau enregistrement sont sur ir.access (domaine sur le droit d'accès), plus via un ir.rule séparé. Exemple : country_id correspondant au pays de l'utilisateur pour un groupe commercial. Permissions et filtres de lignes se configurent au même endroit." },
  }),
  complexQ({
    id: "oep-129",
    module: "studio",
